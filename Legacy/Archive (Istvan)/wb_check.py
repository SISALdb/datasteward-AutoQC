...
20 May 2022
    - Update site_col and sample_col columns check to be compatible with workbook v14
    - Moved geology and rock_age checks from site to entity level and updated options compatible with workbook v14
    - Added vegetation_type, land_use, cover_type checks to entity level
    - Update options 'other' and 'mixed' to 'other (add note to notes sheet)' and 'mixed (add note to notes sheet)' repectively
    - Suppress warnings from openpyxl
    - check that drip_type must be empty when entity is a composite. 
    - check that drip_type is not empty when entity is not a composite.
...
input_dir  = pth
input_file = pth + xls
output_file = pth + "QC_passed_" + xls
...
sample_col = set(['entity_name', 'depth_sample', 'hiatus', 'gap', 'mineralogy', 'arag_corr', 'interp_age', 'interp_age_uncert_pos', 'interp_age_uncert_neg', 'age_model_type', 'modern_reference', 'ann_lam_check', 'dep_rate_check', 'sample_thickness', 'd13C_measurement', 'd13C_precision', 'd18O_measurement', 'd18O_precision', 'Sr_Ca_precision', 'Mg_Ca_measurement', 'Mg_Ca_precision', 'Ba_Ca_measurement', 'Ba_Ca_precision', 'U_Ca_measurement', 'U_Ca_precision', 'P_Ca_measurement', 'P_Ca_precision', 'Sr_isotopes_measurement', 'Sr_isotopes_precision']) - set(sample_tb.columns)
if len(site_col) + len(entity_col) + len(ref_col) + len(date_col) + len(lam_col) + len(sample_col) > 0:
    ...
    sys.exit('This workbook is likely not version 14. The checks cannot be performed.')
...
def check_independent_dependent_col_silent(table, table_name, independent_column, dependent_column):
    ...
    Args:
        table: A pandas dataframe object. Table with the data.
        table_name: string. Name of table (will not be printed in warnings).
        independent_column: string. Name of the independent column
        dependent_column: string. Name of the dependent column
...
y_n_nk_list = ['yes','no','mixed (add note to notes sheet)','other (add note to notes sheet)','unknown']
...
if check_no_values(entity_tb, 'Entity metadata', 'speleothem_type') == 0: 
    warning_ctr += check_values2list(entity_tb, 'speleothem_type', 'Entity metadata', ['stalagmite', 'stalactite', 'flowstone', 'composite', 'mixed (add note to notes sheet)', 'other (add note to notes sheet)', 'unknown'])
else:
    warning_ctr += 1
...
if check_no_values(entity_tb, 'Entity metadata', 'geology') == 0:
    warning_ctr += check_values2list(entity_tb, 'geology', 'Entity metadata', ['limestone', 'dolomite', 'marble', 'dolomitic limestone', 'dolomite limestone', 'marly limestone', 'calcarenite', 'mixed (add note to notes sheet)', 'other (add note to notes sheet)', 'unknown'])
else:
    warning_ctr += 1
if check_no_values(entity_tb, 'Entity metadata', 'rock_age') == 0:
    warning_ctr += check_values2list(entity_tb, 'rock_age', 'Entity metadata', ['Holocene', 'Pleistocene', 'Pliocene', 'Miocene', 'Oligocene', 'Eocene', 'Palaeocene', 'Cretaceous', 'Jurassic', 'Triassic', 'Permian', 'Carboniferous', 'Devonian', 'Silurian', 'Ordovician', 'Cambrian', 'Precambrian', 'mixed (add note to notes sheet)', 'other (add note to notes sheet)', 'unknown'])
else:
    warning_ctr += 1
if check_no_values(entity_tb, 'Entity metadata', 'vegetation_type') == 0:
    warning_ctr += check_values2list(entity_tb, 'vegetation_type', 'Entity metadata', ['evergreen', 'deciduous', 'shrubland', 'grassland', 'sparse', 'barren', 'mixed (add note to notes sheet)','other (add note to notes sheet)', 'unknown'])
else:
    warning_ctr += 1
if check_no_values(entity_tb, 'Entity metadata', 'land_use') == 0:
    warning_ctr += check_values2list(entity_tb, 'land_use', 'Entity metadata', ['water body', 'wetland', 'forest', 'farmland', 'pasture', 'concrete and built up', 'limited or no use', 'mixed (add note to notes sheet)', 'other (add note to notes sheet)', 'unknown'])
else:
    warning_ctr += 1
if check_no_values(entity_tb, 'Entity metadata', 'cover_type') == 0:
    warning_ctr += check_values2list(entity_tb, 'cover_type', 'Entity metadata', ['thick', 'thin', 'patchy', 'barren', 'mixed (add note to notes sheet)', 'other (add note to notes sheet)', 'unknown'])
else:
    warning_ctr += 1
# End of new checks for v14
# Check that the values are from within the list
warning_ctr += check_values2list(entity_tb, 'drip_type', 'Entity metadata', ['seepage flow', 'seasonal drip', 'fast flow', 'mixed (add note to notes sheet)', 'other (add note to notes sheet)', 'unknown', ''], na_rm = True)  # added '' as np.nan was replaced by ''
Entity_y_n_nk = ['d13C', 'd18O', 'd18O_water_equilibrium', 'organics', 'fluid_inclusions', 'mineralogy_petrology_fabric', 'clumped_isotopes', 'noble_gas_temperatures', 'C14', 'ODL', 'Sr_Ca', 'Mg_Ca', 'Ba_Ca', 'U_Ca', 'P_Ca', 'Sr_isotopes', 'trace_elements_datafile', 'trace_elements_metadatafile', 'cave_map']
...
for i in entity_tb.index:
    ...
    if speleothem_type == 'composite':
        print('Entity %s is a composite, checks will be done separately. Ensure that individual records forming this composite are listed in the Notes tab.' %ent_name)
        if drip_type != '':
            warning_ctr += 1
            print('Entity metadata tab: Entity %s is a composite. Drip type must be empty.' %ent_name) 
        ...
        warning_ctr += check_numbers(sample_tb_composite_rm_gap, 'Sample data', 'Sr_Ca_measurement')
        warning_ctr += check_numbers(sample_tb_composite_rm_gap, 'Sample data', 'Sr_Ca_precision')
        warning_ctr += check_numbers(sample_tb_composite_rm_gap, 'Sample data', 'Mg_Ca_measurement')
        warning_ctr += check_numbers(sample_tb_composite_rm_gap, 'Sample data', 'Mg_Ca_precision')
        warning_ctr += check_numbers(sample_tb_composite_rm_gap, 'Sample data', 'Ba_Ca_measurement')
        warning_ctr += check_numbers(sample_tb_composite_rm_gap, 'Sample data', 'Ba_Ca_precision')
        warning_ctr += check_numbers(sample_tb_composite_rm_gap, 'Sample data', 'U_Ca_measurement')
        warning_ctr += check_numbers(sample_tb_composite_rm_gap, 'Sample data', 'U_Ca_precision')
        warning_ctr += check_numbers(sample_tb_composite_rm_gap, 'Sample data', 'P_Ca_measurement')
        warning_ctr += check_numbers(sample_tb_composite_rm_gap, 'Sample data', 'P_Ca_precision')
        warning_ctr += check_numbers(sample_tb_composite_rm_gap, 'Sample data', 'Sr_isotopes_measurement')
        warning_ctr += check_numbers(sample_tb_composite_rm_gap, 'Sample data', 'Sr_isotopes_precision')
        ...
        informative = 0
        informative += check_independent_dependent_col_silent(sample_tb_composite_rm_gap, 'Sample data', 'Sr_Ca_measurement', 'Sr_Ca_precision')
        informative += check_independent_dependent_col_silent(sample_tb_composite_rm_gap, 'Sample data', 'Mg_Ca_measurement', 'Mg_Ca_precision')
        informative += check_independent_dependent_col_silent(sample_tb_composite_rm_gap, 'Sample data', 'Ba_Ca_measurement', 'Ba_Ca_precision')
        informative += check_independent_dependent_col_silent(sample_tb_composite_rm_gap, 'Sample data', 'U_Ca_measurement', 'U_Ca_precision')
        informative += check_independent_dependent_col_silent(sample_tb_composite_rm_gap, 'Sample data', 'P_Ca_measurement', 'P_Ca_precision')
        informative += check_independent_dependent_col_silent(sample_tb_composite_rm_gap, 'Sample data', 'Sr_isotopes_measurement', 'Sr_isotopes_precision')
        ...
        informative += check_independent_dependent_col_silent(sample_tb_composite_rm_gap, 'Sample data', 'Sr_Ca_precision', 'Sr_Ca_measurement')
        informative += check_independent_dependent_col_silent(sample_tb_composite_rm_gap, 'Sample data', 'Mg_Ca_precision', 'Mg_Ca_measurement')
        informative += check_independent_dependent_col_silent(sample_tb_composite_rm_gap, 'Sample data', 'Ba_Ca_precision', 'Ba_Ca_measurement')
        informative += check_independent_dependent_col_silent(sample_tb_composite_rm_gap, 'Sample data', 'U_Ca_precision', 'U_Ca_measurement')
        informative += check_independent_dependent_col_silent(sample_tb_composite_rm_gap, 'Sample data', 'P_Ca_precision', 'P_Ca_measurement')
        informative += check_independent_dependent_col_silent(sample_tb_composite_rm_gap, 'Sample data', 'Sr_isotopes_precision', 'Sr_isotopes_measurement')
        if informative > 0:
            print('Informative: There are trace element measurements provided without precision values.')
        ...
        # warning_ctr += check_independent_dependent_col(sample_tb_composite_rm_gap, 'Sample data', 'Sr_Ca_std', 'Sr_Ca_measurement')
        # warning_ctr += check_independent_dependent_col(sample_tb_composite_rm_gap, 'Sample data', 'Mg_Ca_std', 'Mg_Ca_measurement')
        # warning_ctr += check_independent_dependent_col(sample_tb_composite_rm_gap, 'Sample data', 'Ba_Ca_std', 'Ba_Ca_measurement')
        # warning_ctr += check_independent_dependent_col(sample_tb_composite_rm_gap, 'Sample data', 'U_Ca_std', 'U_Ca_measurement')
        # warning_ctr += check_independent_dependent_col(sample_tb_composite_rm_gap, 'Sample data', 'P_Ca_std', 'P_Ca_measurement')
        # warning_ctr += check_independent_dependent_col(sample_tb_composite_rm_gap, 'Sample data', 'Sr_isotopes_std', 'Sr_isotopes_measurement')
...
# =============================================================================
        ...
        if check_no_values(sample_tb_composite_rm_gap, 'Sample metadata', 'mineralogy', col_dtype_str_set = True) == 0:
            a1 = check_values2list(sample_tb_composite_rm_gap, 'mineralogy', 'Sample data', ['calcite', 'secondary calcite', 'aragonite', 'vaterite', 'mixed (add note to notes sheet)', 'unknown'])
        else:
            warning_ctr += 1
            a1 = 0
        ...
        if (a1 == 0) & (b1 == 0): # If the sample table excluding gaps have mineralogy and arag_corr
            ...
            sample_calcite = sample_tb_composite_rm_gap.loc[(sample_tb_composite_rm_gap['mineralogy'] == 'aragonite') | (sample_tb_composite_rm_gap['mineralogy'] == 'mixed (add note to notes sheet)'), :]
            row_no = sample_calcite.loc[sample_calcite['arag_corr'] == 'not applicable',:].index
            if len(row_no) > 0:
                row_no = str([i+3 for i in row_no]).replace('[', '').replace(']', '')
            ...
            sample_calcite = sample_tb_composite_rm_gap.loc[(sample_tb_composite_rm_gap['mineralogy'] == 'mixed (add note to notes sheet)'), :]
            ...
            if len(row_no) > 0:
                ...
                pass # all are 'not applicable'
#        warning_ctr += check_values2list(sample_tb_composite_rm_gap, 'mineralogy', 'Sample data', ['calcite', 'secondary calcite', 'aragonite', 'vaterite', 'mixed (add note to notes sheet)', 'unknown', ''], na_rm = True)  # added '' as np.nan was replaced by ''
# =============================================================================
#         #
# =============================================================================
        warning_ctr += check_values2list(sample_tb_composite_rm_gap, 'arag_corr', 'Sample data', ['yes', 'no', 'not applicable', 'unknown'])
        warning_ctr += check_values2list(sample_tb_composite_rm_gap, 'age_model_type', 'Sample data', ['linear', 'linear between dates', 'polynomial fit', 'polynomial fit omitting outliers', 'Bayesian', 'Bayesian Bacon', 'Bayesian Bchron', 'StalAge', 'StalAge and other', 'Clam', 'COPRA', 'OxCal', 'mixed (add note to notes sheet)', 'other (add note to notes sheet)', 'unknown'])
        if check_no_values(sample_tb_composite_rm_gap, 'Sample data', 'ann_lam_check', col_dtype_str_set = True) == 0:
            warning_ctr += check_values2list(sample_tb_composite_rm_gap, 'ann_lam_check', 'Sample data', ['14C peak', '14C slope', 'U/Th cycle', 'trace element cycle', 'assumed', 'unknown', 'not applicable', 'other (add note to notes sheet)'])
        else:
            warning_ctr += 1
        ...
        if drip_type == '':
            warning_ctr += 1
            print('Entity metadata tab: Entity %s is not a composite and drip type cannot be empty.' %ent_name) 
...
#            warning_ctr += 1
    ...
    samplecolumnnameslist = ['d18O_measurement', 'd13C_measurement', 'Sr_Ca_measurement', 'Mg_Ca_measurement', 'Ba_Ca_measurement', 'U_Ca_measurement', 'P_Ca_measurement', 'Sr_isotopes_measurement']
    ...
    samplecolumnnameslist2 = ['interp_age_uncert_pos', 'interp_age_uncert_neg', 'sample_thickness', 'd18O_precision',
                             'd13C_precision', 'Sr_Ca_precision', 'Mg_Ca_precision', 'Ba_Ca_precision', 'U_Ca_precision', 'P_Ca_precision', 'Sr_isotopes_precision']
    ...
    informative = 0
    informative += check_independent_dependent_col_silent(sample_tb_rm_hiatus, 'Sample data', 'Sr_Ca_measurement', 'Sr_Ca_precision')
    informative += check_independent_dependent_col_silent(sample_tb_rm_hiatus, 'Sample data', 'Mg_Ca_measurement', 'Mg_Ca_precision')
    informative += check_independent_dependent_col_silent(sample_tb_rm_hiatus, 'Sample data', 'Ba_Ca_measurement', 'Ba_Ca_precision')
    informative += check_independent_dependent_col_silent(sample_tb_rm_hiatus, 'Sample data', 'U_Ca_measurement', 'U_Ca_precision')
    informative += check_independent_dependent_col_silent(sample_tb_rm_hiatus, 'Sample data', 'P_Ca_measurement', 'P_Ca_precision')
    informative += check_independent_dependent_col_silent(sample_tb_rm_hiatus, 'Sample data', 'Sr_isotopes_measurement', 'Sr_isotopes_precision')
    informative += check_independent_dependent_col_silent(sample_tb_rm_hiatus, 'Sample data', 'Sr_Ca_precision', 'Sr_Ca_measurement')
    informative += check_independent_dependent_col_silent(sample_tb_rm_hiatus, 'Sample data', 'Mg_Ca_precision', 'Mg_Ca_measurement')
    informative += check_independent_dependent_col_silent(sample_tb_rm_hiatus, 'Sample data', 'Ba_Ca_precision', 'Ba_Ca_measurement')
    informative += check_independent_dependent_col_silent(sample_tb_rm_hiatus, 'Sample data', 'U_Ca_precision', 'U_Ca_measurement')
    informative += check_independent_dependent_col_silent(sample_tb_rm_hiatus, 'Sample data', 'P_Ca_precision', 'P_Ca_measurement')
    informative += check_independent_dependent_col_silent(sample_tb_rm_hiatus, 'Sample data', 'Sr_isotopes_precision', 'Sr_isotopes_measurement')
    if informative > 0:
        print('Informative: There are trace element measurements provided without precision values.')
    ...
    # warning_ctr += check_independent_dependent_col(sample_tb_rm_hiatus, 'Sample data', 'Sr_Ca_std', 'Sr_Ca_measurement')
    # warning_ctr += check_independent_dependent_col(sample_tb_rm_hiatus, 'Sample data', 'Mg_Ca_std', 'Mg_Ca_measurement')
    # warning_ctr += check_independent_dependent_col(sample_tb_rm_hiatus, 'Sample data', 'Ba_Ca_std', 'Ba_Ca_measurement')
    # warning_ctr += check_independent_dependent_col(sample_tb_rm_hiatus, 'Sample data', 'U_Ca_std', 'U_Ca_measurement')
    # warning_ctr += check_independent_dependent_col(sample_tb_rm_hiatus, 'Sample data', 'P_Ca_std', 'P_Ca_measurement')
    # warning_ctr += check_independent_dependent_col(sample_tb_rm_hiatus, 'Sample data', 'Sr_isotopes_std', 'Sr_isotopes_measurement')
    ...
    for i in set(sample_tb_no_agemodel['entity_name']):
        ...
        depth_ref = entity_tb.loc[entity_tb['entity_name'] == i,'depth_ref'].values
        if (entity_tb['entity_name'] == i).any() != 1:
            # warning_ctr += 1
            # print('Sample tab: Entity %s has no info entity metadata tab. The checking script cannot continue and will terminate here.' %(i))
            continue
        depth_ref = depth_ref[0] # MW debug values[0] IndexError: index 0 is out of bounds for axis 0 with size 0
...
if len(dating_tb.index) > 0:
    ...
    if check_no_values(dating_tb, 'Dating information', 'date_type', True) == 0:
        warning_ctr += check_values2list(dating_tb, 'date_type', 'Dating information', ['C14', 'MC-ICP-MS U/Th', 'ICP-MS U/Th Other', 'Alpha U/Th', 'TIMS', 'U/Th unspecified', 'Cross-dating', 'Multiple methods', 'Event; hiatus', 'Event; gap (composite record)', 'Event; actively forming', 'Event; start of laminations', 'Event; end of laminations', 'unknown', 'other (add note to notes sheet)'])
        ...
        if pass_dateused_warning:
            if pass_entityname_warning:
                if pass_depthdating_warning:
                    ...
                    warning_ctr += check_hiatusgaps_columns(dating_tb, 'Dating information', 'date_type', 'other (add note to notes sheet)', ['entity_name', 'depth_dating', 'date_used', 'date_type', 'corr_age', 'modern_reference'], ['dating_thickness', 'material_dated', 'min_weight', 'max_weight', 'corr_age_uncert_neg', 'corr_age_uncert_pos', 'lab_num'])
...
print('%d warning/s detected' %warning_ctr)
# =============================================================================
# Section 9. Move files to Checked folder
# =============================================================================
# if warning_ctr < 1:
#     # try:
#     #     shutil.move(input_dir, os.path.join(pth, 'checked/', rec))
#     #     print('Files moved into checked folder.')
#     # except IOError:
#     #     print('checked folder is likely missing. Unable to move files automatically into checked folder.')
#     print('QC checks passed')

# =============================================================================
# Section 9. Append file as QC_passed
# =============================================================================
if warning_ctr < 1:
    try:
        ...
        print('An error occured. Unable to append file automatically as QC_passed.')
