...
April 2025 (E, L)
    ...
    - Added trace element measurement/precision checks to Sample data section
    - Added check_independent_dependent_col_silent function (from Istvan's version)
    - Fixed dropdown values to v15 vocabulary (mixed/other → 'add note to notes sheet')
    - Fixed depth_ref lookups (column dropped in v15): auto-detect from data where needed
    - Removed modern_reference from Dating/Lamina laminations sections
    - Updated Dating section dropdowns and column validation lists for v15
    - Merged dating/lamina/reference section checks from Istvan's v14 version into v15
...
input_dir  = os.path.join(script_dir, 'Filled_SISAL_Workbook_v15')
output_dir = os.path.join(script_dir, 'Output')
os.makedirs(output_dir, exist_ok=True)

input_file  = os.path.join(input_dir,  xls)
...
def check_independent_dependent_col_silent(table, table_name, independent_column, dependent_column):
    ...
    Returns:
        integer, count of violations (0 if none)
    """
    sub_tb = table.loc[pd.isnull(table[independent_column]) & pd.notnull(table[dependent_column]), :]
    return sub_tb.shape[0]
...
if loc_warning == True:
    ...
    try:
        ...
        _land_shp = shpreader.natural_earth(resolution='110m', category='physical', name='land')
...
_te_pairs = [
    ...
    ('Mg_Ca_measurement',       'Mg_Ca_precision'),
    ('Ba_Ca_measurement',       'Ba_Ca_precision'),
    ('U_Ca_measurement',        'U_Ca_precision'),
    ('P_Ca_measurement',        'P_Ca_precision'),
    ...
    if a == 0:
        if b == 0:
            ...
            if len(sample_sorted_asc) < 2:
                ...
                mean_agediff = np.mean(np.diff(sample_sorted_asc['interp_age'].values))
                if mean_agediff > 0:
                    depth_ref = 'from top'
...
try:
    ...
    ax.set_extent([lon - span, lon + span, lat - span, lat + span], crs=ccrs.PlateCarree())

    ax.add_feature(cfeature.LAND, facecolor='#f5f0e8', zorder=0)
    ax.add_feature(cfeature.OCEAN, facecolor='#c9e8f5', zorder=0)
    ax.add_feature(cfeature.COASTLINE, linewidth=0.6, zorder=1)
    ax.add_feature(cfeature.BORDERS, linewidth=0.4, linestyle=':', zorder=1)
    ax.add_feature(cfeature.LAKES, facecolor='#c9e8f5', zorder=1)
    ax.add_feature(cfeature.RIVERS, zorder=1)

    ax.plot(lon, lat, marker='*', color='red', markersize=14,
            transform=ccrs.PlateCarree(), zorder=5)
    ax.text(lon + 0.4, lat + 0.4, site_name, fontsize=9, color='red',
            transform=ccrs.PlateCarree(), zorder=5)
    ...
    ax.set_title('Site: %s  (%.4f°N, %.4f°E)' % (site_name, lat, lon), fontsize=11)
